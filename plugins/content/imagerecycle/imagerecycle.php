...
class plgContentImagerecycle extends JPlugin
{
    ...
    public function onContentPrepare($context, &$article, &$params, $limitstart)
    {
          ...
          $optimizeInterval = (int)$componentParams->get('run_periodicity'); 
          $now =(int)strtotime(date('Y-m-d H:i:s'));
          $last_run = $componentParams->get('last_run','');           
          if(!empty($last_run)) { 
              $curInterval= $now - (int)strtotime($last_run);
          }else {
              $curInterval= $now ;
          }
          ...
          if($curInterval/60 > $optimizeInterval && self::$scriptIncluded==0) {
               ...
               $script = "jQuery(document).ready(function($){
                     var optimizeall_auto = '". JUri::root(). "/index.php?option=com_imagerecycle&task=image.optimizeAll&mode=auto';
